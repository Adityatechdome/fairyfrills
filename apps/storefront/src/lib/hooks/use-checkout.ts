...
import { CHECKOUT_FIELDS } from "@/components/cart"
import { HttpTypes } from "@medusajs/types"

async function saveShippingAddressToAccount(order: HttpTypes.StoreOrder) {
...
const DEFAULT_CART_FIELDS = CHECKOUT_FIELDS
...
export const useAutoSelectShippingAndPayment = () => {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: async ({ cart_id, region_id }: { cart_id: string; region_id: string }) => {
      // 1. Fetch shipping options
      const { shipping_options } = await sdk.store.fulfillment.listCartOptions({ cart_id })
      const firstOption = shipping_options?.[0]

      if (firstOption) {
        await sdk.store.cart.addShippingMethod(cart_id, { option_id: firstOption.id })
      }

      // 2. Fetch payment providers
      const { payment_providers } = await sdk.store.payment.listPaymentProviders({
        region_id,
        fields: "id",
      })

      const filtered = payment_providers?.filter((m) => m.id !== "pp_system_default") ?? []
      const firstProvider = filtered[0]

      if (firstProvider) {
        const { cart } = await sdk.store.cart.retrieve(cart_id, { fields: DEFAULT_CART_FIELDS })
        await sdk.store.payment.initiatePaymentSession(cart, { provider_id: firstProvider.id })
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ predicate: queryKeys.cart.predicate })
...
export const useCompleteCartOrder = () => {
  ...
  return useMutation({
    ...
    onSuccess: async (order) => {
      // Save shipping address to customer account if logged in
      await saveShippingAddressToAccount(order)
