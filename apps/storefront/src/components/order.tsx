...
export const OrderLineItem = ({ item, order }: OrderLineItemProps) => {
  const itemSize = (item.metadata as any)?.size as string | undefined
  return (
    <div className="flex items-center gap-4 py-3 border-b border-zinc-200 last:border-b-0">
      <Thumbnail
        thumbnail={item.thumbnail || (item as any).product?.images?.[0]?.url}
      ...
      <div className="flex-1 flex flex-col gap-y-1">
        ...
        {itemSize && (
          <span className="text-sm font-semibold text-zinc-800">Size: {itemSize}</span>
        )}
